...
class WorldHandler {
    ...
    sendClient(user, message) {
        if (user) user.connection.send(message.serialize());
        else {
            console.warn(
                'sendClient called without user set: ' +
    ...
    handleSavePlayerAuctions(msg) {
        console.info(
            'worldHandler, handleSavePlayerAuctions: ' + JSON.stringify(msg)
        );
        const self = this;
        if (!this.world) {
    ...
    handleSavePlayerLooks(msg) {
        console.info(
            'worldHandler, handleSavePlayerLooks: ' /*+JSON.stringify(msg)*/
        );
        const self = this;
        if (!this.world) {
    ...
    handleSaveUserBans(msg) {
        console.info(
            'worldHandler, handleSaveUserBans: ' /*+JSON.stringify(msg)*/
        );
        const self = this;
        if (!this.world) {
    ...
    handleSavePlayerData(msg) {
        ...
        try {
            ...
            if (!this.playerSaveData.hasOwnProperty(playerName)) {
                console.info(
                    'handleSavePlayerData: ' +
                        playerName +
                        ' was not registered in playerSaveData (likely a gameserver reconnect or a save that raced the load handshake) -- self-registering and continuing with save instead of dropping it.'
                );
            }
            ...
            const checkPlayerSaved = function (playerName) {
                try {
                    ...
                    console.error(
                        'handleSavePlayerData - checkPlayerSaved failed for ' +
                            playerName +
                            ': ' +
                            err.stack
                    );
            ...
            DBH.savePlayerUserInfo(
                ...
                playerName,
                data[0],
                function (username, playerName, data) {
    ...
    handlePlayerLoaded(msg) {
        ...
        if (!user) {
            console.warn(
                'handlePlayerLoaded: no pending login for player ' + playerName
            );
            return;
        }
        ...
        this.sendClient(
            user,
            new UserMessages.WorldReady(user, protocol, address, port)
        );
        this.block = false;
    }
    ...
    abandonPendingLogin(playername) {
        ...
        if (!this.pendingLogins.has(playername)) return;

        console.info(
            'abandonPendingLogin: clearing pending login for ' + playername
        );
        this.pendingLogins.delete(playername);
    ...
    createPlayerToWorld(user, username, playername) {
        ...
        const checkLoadDataFull = function (index, data) {
            ...
            if (objData.count === 7) {
                ...
                self.sendMessage(
                    new UserMessages.SendLoadPlayerData(
                        playerName,
                        objData.data
                    )
                );
