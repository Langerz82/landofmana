...
class FormatChecker {
    ...
    check(msg) {
        ...
        if (type === Types.UserMessages.WU_SAVE_PLAYER_DATA) {
            ...
            // uses above.
            if (message[0]) {
                res = stringField(playerNameLenMin, playerNameLenMax).safeParse(
                    message[0]
                );
                if (!res.success) {
                    console.info(
                        'message 0 (player name) failed: ' +
                            describeZodError(res.error)
                    );
                    return false;
                }
            }
            ...
            if (msg) {
                ...
                if (!res.success) {
                    console.info(
                        'message arr1-0 (username/hash record) failed: ' +
                            describeZodError(res.error)
                    );
                    return false;
                }
            ...
            try {
                ...
                console.info(
                    'complete quests JSON parse error :' + JSON.stringify(data)
                );
                return false;
            }
            ...
            }
            {
                const completeQuestsSchema = recordField(
                    z.object({
                        npcid: numberField(0, questNpcIdMax)
                            .nullable()
                            .optional()
                    }),
                    questCountMax
            ...
            for (const rec of arr) {
                if (typeof rec !== 'string') {
                    console.info(
                        'quests format failed: record is not a string.'
                    );
            ...
            if (data) {
                ...
                if (!res.success) {
                    console.info(
                        'message arr1-3 checkFormat failed: ' + res.error
                    );
                    return false;
                }
            ...
            if (data && !this.checkPlayerItems(data, 1)) {
                console.info(
                    'bank data msg1-5 format failed. ' + JSON.stringify(data)
                );
                return false;
            }
            data = message[1][6];
        ...
        if (type === Types.UserMessages.WU_SAVE_PLAYER_AUCTIONS) {
            ...
            if (!Array.isArray(message)) {
                console.error(
                    'WU_SAVE_PLAYER_AUCTIONS: message is not an array.'
                );
                return false;
            }
            ...
            for (const rec of message) {
                ...
                if (!res.success) {
                    console.error(
                        `WU_SAVE_PLAYER_AUCTIONS format failed: ${res.error}`
                    );
        ...
        if (type === Types.UserMessages.WU_SAVE_USER_BANS) {
            ...
            const banFields = [
                stringField(usernameLenMin, usernameLenMax),
                csvNumberField(banDateMin, banDateMax)
            ];
            for (const rec of arr) {
                const res = parseCsvFields(rec, banFields);
                if (!res.success) {
                    console.error(
                        `WU_SAVE_USER_BANS format failed: ${res.error}`
                    );
                    console.warn('record=' + JSON.stringify(rec));
                    return false;
                }
        ...
        if (type === Types.UserMessages.WU_SAVE_PLAYER_LOOKS) {
            ...
            if (!res.success) {
                console.error(
                    `WU_SAVE_PLAYER_LOOKS format failed: ${describeZodError(res.error)}`
                );
