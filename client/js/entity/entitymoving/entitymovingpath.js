...
export function installEntityMovingPath(proto) {
    ...
    proto.getPathIndex = function (step) {
        ...
        if (step < 0 || step >= this.path.length) return null;
    ...
    proto.nextStep = function () {
        ...
        if (this.hasChangedItsPath()) {
            ...
            // FIX: followPath(null) is a silent no-op (see its own `if
            // (!path) return;` above) when requestPathfindingTo() can't find
            // a route to the new destination -- this used to leave
            // `this.path`/`this.step` exactly as they were left by
            // nextStepPath() above (a partially-consumed *old* path toward a
            // destination this entity is no longer trying to reach), so the
            // entity kept walking out the rest of a stale route instead of
            // stopping. Mirrors the `stop` branch below ("no path to
            // continue on" -> forceStop()) for the same situation reached via
            // a redirect instead of path completion/interruption.
            if (path) {
    ...
    proto.move = function (time, orientation, state, x, y) {
        ...
        if (state === 1 && orientation !== Types.Orientations.NONE) {
            ...
            state === 0 ||
            state === 2 ||
            orientation === Types.Orientations.NONE
        ) {
            this.forceStop();
            this.setPosition(x, y);
        }
