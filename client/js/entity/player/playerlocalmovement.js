// Extracted from user.js's User.createPlayer(): the ~14 methods monkey-patched directly onto
// each newly-created Player instance (movement/attack-throttling overrides specific to the
// locally-controlled player, as opposed to Player's own prototype methods shared with remote
// players). Same instance-installer pattern as gamepad/gamepadbuttons.js's
// installGamepadButtonsX(self) functions - these are per-instance closures, not prototype
// methods, so they're applied once per player instance rather than installed onto a shared
// prototype.
import EntityMoving from '../entitymoving/entitymoving.js';
import Timer from '../../timer.js';

/* global Types, game, log, G_LATENCY, G_ROUNDTRIP, G_UPDATE_INTERVAL */

export function installLocalPlayerOverrides(player) {
    player.forceStop = function () {
        ...
        if (!this.isMovingPath()) {
            if (this.key_move_callback) this.key_move_callback(0);
        }
        ...
        const queuedOrientation =
            typeof this.stopKeyMove === 'number' ? this.stopKeyMove : null;
    ...
    player.scheduleMoveRetry = function (
        orientation,
        delay,
        isStillWanted,
        recheckThrottle
