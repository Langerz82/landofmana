    ...
    display() {
        ...
        const callback = function () {
            ...
            switch ($('#lbselect').val()) {
                ...
                case 'pkd':
                    $.each(leaderJSON, function (key, value) {
                        const pkd =
                            value.pd > 0
                                ? Number(value.pk / value.pd).toFixed(2)
                                : 0;
                        if (pkd > 0) leaders.push({ key: key, value: pkd });
                    });
                    break;
                case 'pd':
                    $.each(leaderJSON, function (key, value) {
                ...
                case 'tkd':
                    $.each(leaderJSON, function (key, value) {
                        const tkd =
                            value.td > 0
                                ? Number(value.tk / value.td).toFixed(2)
                                : 0;
                        if (tkd > 0) leaders.push({ key: key, value: tkd });
                    });
                    break;
            }
            ...
            for (let i = recStart; i < recEnd; ++i) {
                ...
                if (i === playerIndex)
                    lbdata +=
                        '<tr class="lbplayer"><td>' +
                        (i + 1) +
                        '</td><td>' +
                        Utils.escapeHtml(leader.key) +
                        '</td><td>' +
                        leader.value +
