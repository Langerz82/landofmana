...
export default class InventoryHandler {
    ...
    setInventory(itemArray) {
        for (let item of itemArray) {
            ...
            else item.name = ItemTypes.KindData[kind].name;
    ...
    getItemInventorySlotByKind(kind) {
        for (let i = 0; i < this.maxNumber; i++) {
            ...
            if (item && kind === item.itemKind) return i;
        }
    }
    ...
    splitItem(type, slot) {
        ...
        if (
            (this.isStackitem(item) && !item2) ||
            (this.isStackitem(item, true) &&
                item2 &&
    ...
    isStackitem(item, maxStack) {
        return (
            ItemTypes.isStackedItem(item.itemKind) &&
            item.itemNumber > 1 &&
            (!maxStack || (maxStack && item.itemNumber < 100))
        );
    }
    ...
    useItem(type, item) {
        ...
        if (ItemTypes.isConsumableItem(kind)) {
            ...
            if (
                kind &&
                this.dialog.coolTimeCallback === null &&
    ...
    _moveItem(obj, type, slot, start) {
        ...
        if (start && obj === null) {
            return {
                ...
                type: type,
                slot: slot,
                item: this.dialog.getItem(type, slot)
            };
        }
