...
export function installGameUI(proto) {
    ...
    proto.resize = function (zoomMod) {
        ...
        if (this.storeDialog.visible) this.storeDialog.rescale();
    ...
    proto.showNotification = function (data) {
        ...
        if (message && data.length > 0) message = lang.data[text].format(data);
        ...
        if (group.indexOf('SHOP') === 0 || group.indexOf('INVENTORY') === 0) {
