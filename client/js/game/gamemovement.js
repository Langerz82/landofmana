...
export function installGameMovement(proto) {
    ...
    proto.findPath = function (character, x, y, ignoreList, includeList) {
        ...
        if (!this.pathfinder || !character) {
            ...
            log.error(
                'game.findPath - Error while finding the path to ' +
                    x +
                    ', ' +
                    y +
                    ' for ' +
        ...
        if (
            ...
            log.error(
                'game.findPath - path coordinates outside of dimensions.'
            );
            return null;
        }
        ...
        let gridExtra = Math.max(
            Math.abs(fpS[0] - fpE[0]),
            Math.abs(fpS[1] - fpE[1])
        );
        ...
        if (!gridPath) {
            ...
            gridPath = this.pathfinder.findShortPath(
                sgrid,
                shortGrid.minX,
                shortGrid.minY,
                fspS,
        ...
        if (!gridPath) {
            ...
            if (path) {
                ...
                shortGrid.minY = 0;
                log.info(
                    'game.findPath - validpath-mp5:' + JSON.stringify(path)
                );
            }
        }
    ...
    proto.moveCharacter = function (char, x, y, skipOverlap, skipGridCheck) {
        ...
        if (!skipGridCheck) {
            ...
            const check = o === 1 || o === 2 ? my === midTile : mx === midTile;
    ...
    proto.isOverlapping = function (entity, x, y) {
        ...
        for (let k in entities) {
            ...
            if (entity instanceof Player && entity.holdingBlock === entity2)
                continue;
            if (!entity2 || entity === entity2) continue;
            if (entity2.isDead || entity2.isDying) continue;

            if (
                !entity2.isWithinDist(entity.x, entity.y, G_TILESIZE - 1) &&
