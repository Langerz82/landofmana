...
export function installGameInteractionTarget(proto) {
    proto.makePlayerInteractNextTo = function () {
        ...
        if (p.isDying || p.isDead) return;

        if (this.tryShowDialogue()) return;
    ...
    proto.tryInteractFacedEntity = function () {
        ...
        if (!(p.isAdjacentEntity(entity) && p.isFacingEntity(entity)))
            return false;
    ...
    proto.tryInteractAdjacentEntity = function () {
        ...
        const candidates = this.camera
            .forEachInScreenArray(p)
            .filter(
                (e) => e && !e.isDying && !e.isDead && p.isInReach(e.x, e.y)
            );

        if (candidates.length === 0) return false;

        candidates.sort(
            (a, b) => Utils.realDistanceXY(p, a) - Utils.realDistanceXY(p, b)
        );
    ...
    proto.tryInteractHarvestTiles = function () {
        ...
        const pos = p.nextTile();
        if (fnProcessTile(pos[0], pos[1])) return true;

        const spots = p.getSpotsAround(p, 1);
        for (const spot of spots) {
            if (pos[0] === spot.x && pos[1] === spot.y) continue;
            if (fnProcessTile(spot.x, spot.y)) return true;
        }
    ...
    proto.tryInteractExistingTarget = function () {
        ...
        if (p.canReachTarget()) {
            if (!p.isMoving()) p.lookAtEntity(target);
            return this.processTarget();
        }
        return false;
    };
    ...
    proto.tryInteractClosestEntity = function () {
        ...
        if (prevTarget !== p.target && !p.canReachTarget()) return false;
    ...
    proto.playerTargetClosestEntity = function (inc) {
        ...
        const entity = this.entityTargetClosestEntity(
            p,
            inc,
            p.targetIndex,
            excludeTypes
    ...
    proto.entityTargetClosestEntity = function (
        entity,
        inc,
        index,
        excludeTypes
        ...
        entities = entities.filter(
            (entity) =>
                !(
                    excludeTypes.includes(entity.type) ||
                    entity.isDying ||
                    entity.isDead
                )
        );
        ...
        entities.sort(function (a, b) {
            return a.playerDistance > b.playerDistance ? 1 : -1;
        });
