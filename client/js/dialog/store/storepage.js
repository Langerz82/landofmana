...
export class StorePage extends TabPage {
    ...
    open(min, max) {
        ...
        let cond = function (item) {
            return ItemTypes.isConsumableItem(item.kind);
        };
        if (this.itemType === 2)
            cond = function (item) {
        ...
        while (--i >= 0) {
            ...
            if (!cond(item)) this.items.splice(this.items.indexOf(item), 1);
        }
    }
    ...
    reload() {
        ...
        for (
            let index = this.pageIndex * this.rackRows;
            index <
            Math.min((this.pageIndex + 1) * this.rackRows, this.items.length);
            index++
        ) {
...
export class StorePotionPage extends StorePage {
    constructor(parent, scale) {
        super(parent, '#storeDialogStore', 1, null, scale, 0); // FIX (conversion): this._super(...) -> super(...)
    }
}

export class StoreArmorPage extends StorePage {
    constructor(parent, scale) {
        super(parent, '#storeDialogStore', 2, null, scale, 1); // FIX (conversion): this._super(...) -> super(...)
    }
}

export class StoreWeaponPage extends StorePage {
    constructor(parent, scale) {
        super(parent, '#storeDialogStore', 3, null, scale, 2); // FIX (conversion): this._super(...) -> super(...)
    }
}
