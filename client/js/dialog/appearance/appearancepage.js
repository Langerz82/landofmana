...
export class AppearancePage extends TabPage {
    ...
    onData() {
        this.items = [];
        let categoryType;
        if (!game || !game.player || !game.player.appearances) return;

        if (this.itemType === 0) categoryType = 'armor';
        if (this.itemType === 1) categoryType = 'weapon';

        if (game.player.isArcher()) {
            if (this.itemType === 0) categoryType = 'armorarcher';
            if (this.itemType === 1) categoryType = 'weaponarcher';
        }
        ...
        if (this.parent.getPageIndex() !== 0) this.parent.setPageIndex(0);
    }
    ...
    reload() {
        ...
        for (
            ...
            index < (this.pageIndex + 1) * this.rackSize;
            index++
        ) {
            const rack = this.racks[index - this.pageIndex * this.rackSize];
