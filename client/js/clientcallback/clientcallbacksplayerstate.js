...
export function installClientCallbacksPlayerState(proto) {
    ...
    proto.onPlayer = function (data) {
        ...
        if (itemCount > 0) {
            ...
            const itemArray = Utils.ArrayParseInt(
                data.splice(0, itemCount * 6)
            );
        ...
        if (questCount > 0) {
            ...
            for (let i = 0; i < questCount; ++i) {
                const index = i * 13;
                p.quests[questArray[index]] = new Quest(
                    questArray.slice(index, index + 13)
                );
            }
        }
        ...
        if (achieveCount > 0) {
            ...
            for (let i = 0; i < achieveCount; ++i) {
                ...
                achievement = new Achievement(
                    achieveArray.slice(index, index + 7)
                );
