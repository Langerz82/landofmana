...
export function installClientCallbacksSpawn(proto) {
    proto.onSpawnItem = function (data, item) {
        ...
        let sprite = null;
        if (ItemTypes.isLootItem(kind)) sprite = game.sprites['itemloot'];
        else sprite = game.sprites[ItemTypes.KindData[item.kind].spriteName];
    ...
    proto.spawnEntity = function (data, entity) {
        ...
        const droppedMove =
            game.unknownEntityDrops && game.unknownEntityDrops[id];
        if (droppedMove) {
            const spawnX = Number(data[5]),
                spawnY = Number(data[6]);
            const dist = Math.hypot(
                spawnX - droppedMove.x,
                spawnY - droppedMove.y
            );
            console.warn(
                '[teleport-debug] entity id=' +
                    id +
                    ' had a dropped ' +
                    droppedMove.source +
                    ' to (' +
                    ...
                    droppedMove.y +
                    ') ' +
                    (Date.now() - droppedMove.droppedAt) +
                    'ms before spawning at (' +
                    spawnX +
                    ',' +
                    spawnY +
                    ') - distance=' +
            ...
            entity.onRequestPath(function (x, y) {
                ...
                if (!game.pathfinder.isValidPath(path)) {
                    try {
                        ...
                        console.error('invalidpath: ' + JSON.stringify(path));
                    }
                }
                return path;
            });
    ...
    proto.onDespawnEntity = function (data) {
        if (game.mapIndex !== Number(data[1])) return;
        ...
        if (entity) {
            ...
            if (entity instanceof Mob)
                ...
                else entityName = ItemTypes.KindData[entity.kind].name;
            }
            log.info(
                'Despawned ' +
                    entityName +
