...
export function installGameClientSend(proto) {
    ...
    proto.sendLoginPlayer = function (playername, playerhash) {
        this.sendMessage([
            Types.Messages.CW_LOGIN_PLAYER,
            playername,
            playerhash
        ]);
    };
    ...
    proto.sendChat = function (text) {
        this.sendMessage([Types.Messages.CW_CHAT, text]);
    };
    ...
    proto.sendStoreEnchant = function (type, index) {
        // type 0 = Inventory, 2 = Equipment.
        this.sendMessage([Types.Messages.CW_STORE_MODITEM, 1, type, index]);
    };

    proto.sendStoreRepair = function (type, index) {
        // type 0 = Inventory, 2 = Equipment.
    ...
    proto.sendPartyInvite = function (name, status) {
        // 0 for request, 1, for yes, 2 for no.
        this.sendMessage([Types.Messages.CW_PARTY, 1, name, status]);
    };

    proto.sendPartyKick = function (name) {
        this.sendMessage([Types.Messages.CW_PARTY, 2, name, 0]);
    };

    proto.sendPartyLeader = function (name) {
        this.sendMessage([Types.Messages.CW_PARTY, 3, name, 0]);
    };
