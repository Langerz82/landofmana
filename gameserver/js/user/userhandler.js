    ...
    switch (action) {
        case Types.UserMessages.UW_LOAD_PLAYER_DATA:
            ...
            return (
                ...
                message[1].length === 7 &&
                Array.isArray(message[1][0]) &&
                message[1][0].length >= 4
            );
...
class UserHandler {
    constructor(main, server, world, connection) {
        ...
        this.connection.listen(function (message) {
            ...
            if (!isValidUserMessage(action, message)) {
                console.warn(
                    'userHandler: rejected malformed message for action ' +
                        action +
                        ': ' +
    ...
    handleLoadUserBans(msg) {
        ...
        if (this.world.ban) this.world.ban.loadBans(msg);
        else {
        ...
        try {
            ...
            for (let i = 0; i < dataJSON.length; i++) {
                ...
                if (questData) {
                    ...
                    quest.data = QuestData.Data.hasOwnProperty(quest.id)
                        ? QuestData.Data[quest.id]
                        : null;
    ...
    sendToUserServer(msg) {
        if (this.connection) this.connection.send(msg.serialize());
        else {
            this.userHandlerPackets.push(msg);
            console.info(
                'userHandler: sendToUserServer called without connection being set: ' +
    ...
    sendWorldPlayerCount(count, maxCount) {
        this.sendToUserServer(
            new UserMessages.UpdatePlayerCount(count, maxCount)
        );
    }
    ...
    sendPlayerLogout(player) {
        this.sendToUserServer(
            new UserMessages.playerLoggedIn(0, player.user.name, player.name)
        );
    }
