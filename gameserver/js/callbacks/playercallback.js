...
class PlayerCallback {
    ...
    setCallbacks(entity) {
        ...
        const stopPathing = function (p, x, y) {
            ...
            if (G_DEBUG) console.info('onStopPathing');
            p.setPosition(x, y);
            //p.forceStop();
        ...
        const abortPathing = function (p, path, x, y) {
            const dist = self.entities.pathfinder.getPathSubDistance(
                path,
                x,
                y
            );
            if (
                self.entities.pathfinder.isDistanceTooFast(
                    p.tick,
                    dist,
                    p.startMovePathTime
                )
        ...
        p.onAbortPathing(function (path, x, y) {
            if (G_DEBUG) console.info('onAbortPathing');
        ...
        p.checkStopDanger = function (c, o) {
            ...
            if (res) {
                ...
                if (G_DEBUG) {
                    console.info(
                        'checkStopDanger, WARN - PLAYER ' +
                            c.id +
                            ' not stopping.'
                    );
                    console.info(
                        'checkStopDanger, orientation: ' +
                            Utils.getOrientationString(o)
                    );
        ...
        p.checkPathInterrupt = function (x, y) {
            if (!this.isMovingPath()) return false;

            const pathfinder = this.map.entities.pathfinder;

            if (!pathfinder.isInPath(this.path, [x, y])) return true;
            ...
            if (!dist) {
                ...
                if (G_DEBUG) {
                    console.info(
                        'checkPathInterrupt, getPathSubDistance = not found.'
                    );
                    console.info(
                        'checkPathInterrupt, getPathSubDistance: path:' +
                            JSON.stringify(this.path)
                    );
                    console.info(
                        'checkPathInterrupt, getPathSubDistance: x:' +
                            x +
                            ',y:' +
                            y
                    );
            ...
            const res = pathfinder.isDistanceTooFast(
                this.tick,
                dist,
                this.startMovePathTime
            );
            return res;
        };
        ...
        p.checkStartMove = function (x, y) {
            if (this.mapStatus < 2) return false;
            ...
            if (this.map.isColliding(x, y)) {
                if (G_DEBUG)
                    console.info(
                        'checkStartMove - char.isColliding(' +
                            this.id +
                            ',' +
                            x +
                            ',' +
            ...
            if (this.x === x && this.y === y) {
                if (G_DEBUG) console.info('checkStartMove - same coords.');
                return true;
            }
            ...
            if (this.isMoving()) {
                ...
                if (G_DEBUG)
                    console.info(
                        'playercallback, checkStartMove, isMoving - path: ' +
                            JSON.stringify(path)
                    );
                ...
                if (!pathfinder.isValidPath(path)) {
                    if (G_DEBUG)
                        console.info(
                            'playercallback, checkStartMove, isMoving - isValidPath false.'
                        );
                    return false;
                }
                if (!pathfinder.isValidGridPath(this.map.grid, path, true)) {
                    if (G_DEBUG)
                        console.info(
                            'playercallback, checkStartMove, isMoving - isValidGridPath false.'
                        );
                    return false;
                }
                ...
                if (G_DEBUG)
                    console.info(
                        'playercallback, checkStartMove, isMoving - isDistanceTooFast.'
                    );
                return !pathfinder.isDistanceTooFast(
                    this.tick,
                    dist,
                    this.startMoveTime
                );
            ...
            if (G_DEBUG) {
                console.info(
                    'checkStartMove - id:' + this.id + ' different coords.'
                );
                console.info(
                    'checkStartMove - id:' +
                        this.id +
        ...
        p.correctMove = function (x, y) {
            if (
                ...
                console.warn(
                    'ERROR - MOVING NOT SYNCHED PROPERLY, FORCING CLIENT UPDATE'
                );
