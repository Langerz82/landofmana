...
class MobCallback {
    ...
    setCallbacks(entity) {
        ...
        entity.onRequestPath(function (x, y) {
            ...
            if (this.target) ignored.push(this.target);
        ...
        entity.onStopPathing(function (x, y) {
            ...
            if (G_DEBUG)
                console.info(
                    'RETURNING-DEBUG onStopPathing id=' +
                        this.id +
                        ' aiState=' +
                        this.aiState +
                        ' x=' +
            ...
            if (!this.hasTarget()) this.setAiState(mobState.IDLE);
        ...
        entity.onAbortPathing(function (path, x, y) {
            ...
            if (G_DEBUG)
                console.info(
                    'RETURNING-DEBUG onAbortPathing id=' +
                        this.id +
                        ' aiState=' +
                        this.aiState +
                        ' x=' +
            ...
            if (!this.target) this.setAiState(mobState.IDLE);
        });
        ...
        entity.onKilled(function (attacker, damage) {
            if (attacker instanceof Player) {
                ...
                this.world.taskHandler.processEvent(
                    attacker,
                    PlayerEvent(Types.EventType.DAMAGE, this, damage)
                );
            }
        });
        ...
        entity.onDeath(function (attacker) {
            if (attacker instanceof Player) {
                this.world.taskHandler.processEvent(
                    attacker,
                    PlayerEvent(Types.EventType.KILLMOB, this, 1)
                );
            }
