...
class MobAI {
    ...
    checkHitAggro(mob, sEntity) {
        if (mob.isStunned || mob.isAttacking()) return;

        if (mob.aiState !== mobState.IDLE) return;

        if (mob.hasTarget()) return;

        if (mob.isAttackedBy(sEntity)) this.aggroPlayer(mob, sEntity);
        return;
    }
    ...
    checkHit(mob) {
        ...
        if (mob.aiState !== mobState.ATTACKING) return;

        if (mob.isStunned || !mob.target || mob.freeze) return;
        ...
        if (mob.canAttack()) {
            ...
            if (G_DEBUG) console.info('mob - Is Attacking');
    ...
    update() {
        ...
        for (const mob of mobs) {
            if (mob.isDead || mob.freeze) continue;
            ...
            // that one mob.
            if (mob.aiState === mobState.RETURNING) continue;

            if (mob.canAggro()) this.checkAggro(mob);
            if (mob.hasTarget()) {
                this.checkChase(mob);
                this.checkHit(mob);
            }
    ...
    checkChase(mob) {
        ...
        if (mob.aiState === mobState.RETURNING) return;
        ...
        //console.info("mobAI - checkChase.");
        if (!(
            mob.aiState === mobState.AGGRO || mob.aiState === mobState.CHASING
        )) {
        ...
        if (!mob.canReach(target)) {
            ...
            if (mob.isMoving() && target.isMoving()) {
                ...
                if (G_DEBUG)
                    console.info(
                        mob.id + ' monster and target is moving so abort.'
                    );
                return;
            }

            if (mob.canMoveAI()) {
                if (this.checkReturn(mob)) return;
                ...
                if (
                    !target.isMoving() &&
                    mob.ptx === mob.target.x &&
                    mob.pty === mob.target.y
                ) {
            ...
            if (!mob.isMovingPath()) {
                ...
                mob.forceStop();
                if (G_DEBUG) console.info(mob.id + ' within range');
    ...
    handleHurt(mob) {
        ...
        if (!mob || !mob.target || mob.freeze) return;
        ...
        if (mob.target.stats.hp > 0) {
            ...
            if (G_DEBUG) console.info('handleHurt - mob');
    ...
    checkReturn(entity) {
        if (
            entity.aiState !== mobState.CHASING &&
        ...
        if (
            entity.distanceToPos(entity.spawnX, entity.spawnY) >=
                16 * G_TILESIZE ||
            (et && entity.distanceToPos(et.x, et.y) >= 12 * G_TILESIZE)
        ) {
            if (G_DEBUG) console.info('RETURN TO SPAWN!');
            entity.returnToSpawn();
            return true;
        }
        return false;
