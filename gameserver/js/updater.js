...
class Updater {
    constructor(ws, map) {
        ...
        this.playerPathXF = function (c, m) {
            const x = c.x + m;
            const y = c.y;
            if (x % self.whoDist === 0) c.map.entities.processWho(c);
            c.setPosition(x, y);
            return c.nextStep();
        };
        ...
        this.playerPathYF = function (c, m) {
            const x = c.x;
            const y = c.y + m;
            if (y % self.whoDist === 0) c.map.entities.processWho(c);
            c.setPosition(x, y);
            return c.nextStep();
        };
    ...
    update() {
        ...
        for (const entity of this.map.entities.characters.values()) {
            ...
            if (entity instanceof Player) {
                ...
                if (entity.path) this.updatePlayerPathMovement(entity);
            } else {
                if (entity.path) this.updateCharacterPathMovement(entity);
            }
            m = entity.movement;
            if (m && m.inProgress) {
                m.step(this.time);
    ...
    checkCollide(c, x, y) {
        if (c instanceof Player) {
            if (c.map.isColliding(x, y)) return true;
            if (c.holdingBlock) {
                const pos = c.nextTile();
                if (c.map.isColliding(pos[0], pos[1])) return true;
            }
            return false;
        }
    }
    ...
    updateCharacterPathMovement(c) {
        ...
        if (canMove) {
            if (o === Types.Orientations.LEFT) {
                c.movement.start(self.charPathXF, null, -tick);
            } else if (o === Types.Orientations.RIGHT) {
                c.movement.start(self.charPathXF, null, tick);
            } else if (o === Types.Orientations.UP) {
                c.movement.start(self.charPathYF, null, -tick);
