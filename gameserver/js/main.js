...
// constants.js -- main.js is the process entry point (config loading,
// bootstrap, shutdown), not a natural home for values that ~25 unrelated
// files across area/, entity/, map/, packets/, etc. need just to get a tile
// size or a debug flag. See constants.js for the full history/rationale.
// G_DEBUG is re-exported here (via `export { G_DEBUG }` below) purely so
// main() can read the post-startup value in its own console.info call;
// setG_DEBUG() is what actually flips it once config.json is read.
import { setG_DEBUG, G_DEBUG } from './constants.js';
export { G_DEBUG };
