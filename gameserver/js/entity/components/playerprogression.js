...
class PlayerProgression {
    ...
    getState() {
      ...
      const sprite1 = entity.getSprite(0), sprite2 = entity.getSprite(1);

      const state = [entity.level,
        entity.stats.hp,
        entity.stats.hpMax,
        0,
        sprite1, sprite2,
    ...
    incExp(gotexp)
    {
      ...
      entity.stats.exp.base = parseInt(entity.stats.exp.base) + parseInt(incExp);
      const lvl = this.getLevel();
      entity.sendPlayer(new Messages.Stat("exp.base", entity.stats.exp.base, incExp));
    ...
    incAttackExp(gotexp){
      ...
      const prevLvl = this.getAttackLevel();
    	entity.stats.exp.attack = parseInt(entity.stats.exp.attack) + parseInt(incExp);
      const lvl = this.getAttackLevel();
      if(prevLvl !== lvl) {
      	entity.sendPlayer(new Messages.LevelUp("attack", lvl, entity.stats.exp.attack));
      }
      return incExp;
    }
    ...
    incDefenseExp(gotexp){
      ...
      const prevLvl = this.getDefenseLevel();
    	entity.stats.exp.defense = parseInt(entity.stats.exp.defense) + parseInt(incExp);
      const lvl = this.getDefenseLevel();
      if(prevLvl !== lvl) {
      	entity.sendPlayer(new Messages.LevelUp("defense", lvl, entity.stats.exp.defense));
      }
      return incExp;
    }
    ...
    incWeaponExp(gotexp){
      ...
      if (!entity.stats.exp.hasOwnProperty(type))
        return null;
    ...
    levelUp(prevLevel) {
      ...
      entity.setEpMax();
	    ...
	    entity.sendPlayer(new Messages.LevelUp("base", entity.level, entity.stats.exp.base));
    }
