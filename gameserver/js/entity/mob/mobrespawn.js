...
class MobRespawn {
    ...
    createBoss(multi) {
        ...
        for (const kind in entity.drops) {
            if (ItemTypes.isEquipment(kind)) entity.drops[kind] *= multi;
        }
    }
    ...
    returnToSpawn() {
        ...
        if (entity.aiState === mobState.RETURNING) return;

        entity.forceStop();
        entity.setAiState(mobState.RETURNING);

        if (entity.hasTarget()) entity.clearTarget();
        entity.forgetEveryone();
        ...
        if (entity.x === entity.spawnX && entity.y === entity.spawnY) {
            if (G_DEBUG)
                console.info(
                    'RETURNING-DEBUG already-at-spawn id=' + entity.id
                );
        ...
        if (G_DEBUG)
            console.info(
                'RETURNING-DEBUG path-requested id=' +
                    entity.id +
                    ' pathLen=' +
                    (entity.path ? entity.path.length : 0)
            );
    ...
    setDrops() {
        ...
        for (const kind in ItemData.Kinds) {
            ...
            if (ItemTypes.isEquipment(kind)) {
                if (diff >= 0 && diff < 5) entity.drops[kind] = 1;
                if (diff >= -5 && diff < 0) entity.drops[kind] = 2;
                if (diff >= -10 && diff < -5) entity.drops[kind] = 5;
            }
        }

        if (entity.level >= 15 && entity.level < 25) entity.drops[310] = 250;
        if (entity.level >= 25 && entity.level < 35) entity.drops[311] = 250;
        if (entity.level >= 35 && entity.level < 45) entity.drops[312] = 250;
        if (entity.level >= 45) entity.drops[313] = 250;

        // Potions.
        if (entity.level < 20) entity.drops[34] = 250;
        else entity.drops[36] = 250;
    }
}
