...
class Player extends Character {
    ...
    sendPlayerToClient() {
        ...
        // Send All Quests
        const quests = self.quests.quests.filter(function (q) {
            return q.status !== Types.QuestStatus.COMPLETE;
        });
        ...
        for (
            let achieveIndex = 0;
            achieveIndex < achievements.length;
            ++achieveIndex
        ) {
    ...
    sendChangePoints(health, energy) {
        this.map.entities.sendNeighbours(
            this,
            new Messages.ChangePoints(this, health, energy)
        );
    }
    ...
    sendToUserServer(msg) {
        if (this.world) this.world.send(msg.serialize());
        else {
            console.warn(
                'Player, sendToUserServer called without world being set. ' +
                    JSON.stringify(msg)
            );
        }
    }
    ...
    save(update) {
        ...
        if (this.connection.worldHandler)
            ...
            console.warn(
                'Player, save called without worldHandler being set. '
            );
        }
    }
    ...
    movePath(data, path) {
        ...
        if (G_DEBUG) console.info('set path');
        ...
        this.forceStop();
        if (!this.map.entities.pathfinder.isValidPath(path)) return;
    ...
    move(nm) {
        ...
        if (G_DEBUG) console.info('nm:' + JSON.stringify(nm));
      ...
      return;
        ...
        try { throw new Error(); } catch (e) { console.error(e.stack); }
            ...
            if (a || b) {
                ...
                if (G_DEBUG) {
                    console.info(
                        'player.move, resetMove - x:' + x + ', y:' + y
                    );
                    console.info(
                        'player.move, resetMove - this.x:' +
                            this.x +
                            ', this.y:' +
                            this.y
                    );
    ...
    broadcastSprites() {
        ...
        this.setSprite(1, s2);
        this.packetHandler.broadcast(
            new Messages.setSprite(this, s1, s2),
            false
        );
    }

    sendCurrentMove() {
        const msg = new Messages.Move(
            this,
            this.orientation,
            0,
            this.x,
    ...
    isInScreen(pos) {
        return (
            ~~(Math.abs(this.x - pos[0]) / G_TILESIZE) <=
                ~~(G_SCREEN_WIDTH / 2) &&
            ~~(Math.abs(this.y - pos[1]) / G_TILESIZE) <=
                ~~(G_SCREEN_HEIGHT / 2)
        );
    }
    ...
    setSprite(type, id) {
        if (type === 0) {
            if (this.isArcher()) this.sprites[2] = id;
            else this.sprites[0] = id;
        } else if (type === 1) {
            if (this.isArcher()) this.sprites[3] = id;
            else this.sprites[1] = id;
        }
    }
    ...
    getSprite(type) {
        ...
        if (type === 1) {
            ...
            if (item) {
                ...
                if (this.isArcher()) return 50;
                else return 0;
            }
        } else if (type === 0) {
            if (this.isArcher()) return this.sprites[2];
            else return this.sprites[0];
        }
    }
    ...
    handleTeleport() {
        if (this.on_teleport_callback) this.on_teleport_callback();
    }

    hasMoveThrottled(delay) {
        if (Date.now() - this.lastMoveThrottle < delay) return true;
