...
class Map {
    constructor(world, id, name, filepath, filenameCollision) {
        ...
        this.world = world;
        console.info(
            'filepath: ' + filepath + ',filenameCollision: ' + filenameCollision
        );
        const self = this;
        this.name = name;
        this.isLoaded = false;
    ...
    loadCollisionGrid(collisions) {
        ...
        for (let i = 0; i < this.height; ++i) {
            this.grid[i] = new Uint8Array(
                collisions.slice(i * this.width, (i + 1) * this.width)
            );
        }
        collisions = null;
    ...
    initConnectedGroups(doors) {
        ...
        _.each(doors, function (door) {
            ...
            const connectedGroupId = self.getGroupIdFromPosition(
                door.tx,
                door.ty
            );
    ...
    getRandomPositionCollide(collide) {
        ...
        let tries = 0;
        do {
            pos.x = Utils.randomRangeInt(
                0 - collide[0],
                self.width - 1 - collide[2]
            );
            pos.y = Utils.randomRangeInt(
                0 - collide[1],
                self.height - 1 - collide[3]
            );
            if (self.isValidPosition(pos.x, pos.y)) break;
        } while (tries++ < 25);
    ...
    getRandomPositionArea(x1, x2, y1, y2) {
        ...
        do {
            ...
            if (!this.isColliding(pos.x, pos.y)) break;
        } while (tries++ < 20);
    ...
    getRandomPosition() {
        ...
        do {
            ...
            if (!self.isColliding(pos.x, pos.y)) break;
        } while (tries++ < 20);
    ...
    getSubIndex(x, y) {
        return ~~(
            (y / (this.chunkHeight * this.tilesize)) * this.chunkHeight +
    ...
    isHarvestTile(pos, type) {
        if (G_DEBUG) console.info('isHarvestTile');
        //var gx = pos.x >> 4, gy = pos.y >> 4;
        if (this.isOutOfBounds(pos.gx, pos.gy)) return false;
        ...
        if (!HARVEST_TILE_TYPES.hasOwnProperty(type)) return false;

        let res = false;
        if (Array.isArray(tiles)) {
            res = HARVEST_TILE_TYPES[type].some(function (tile) {
