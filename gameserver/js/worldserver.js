...
class World {
    constructor(id, maxPlayers, socket) {
        ...
        self.onPlayerConnect(function (player) {
            ...
            if (self.players.indexOf(player) < 0) {
                ...
                self.userHandler.sendWorldPlayerCount(
                    self.players.length,
                    self.maxPlayers
                );
            }
        ...
        self.onPlayerEnter(function (player) {
            ...
            player.map.entities.sendBroadcast(
                new Messages.Notify('MAP', 'MAP_ENTERED', [
                    player.name,
                    player.map.name
                ]),
                true
            ...
            player.packetHandler.onExit(function (player) {
                ...
                if (self.removed_callback) self.removed_callback(player);
            ...
            if (self.added_callback) self.added_callback();
        });
        ...
        Scheduler.every(function () {
            Utils.forEach(self.notify, function (notify) {
                if (Date.now() - notify.lastTime >= notify.interval * 60000) {
                    self.sendWorld(
                        new Messages.Notify('NOTICE', notify.textid)
                    );
    ...
    savePlayers(update) {
        if (this.userHandler) {
            ...
            console.warn(
                'worldServer save: no user server connection aborting save.'
            );
            return false;
        }
    ...
    save(update) {
        if (this.savePlayers(update)) this.PLAYERS_SAVED = true;

        if (this.auction.save(this)) this.AUCTIONS_SAVED = true;

        if (this.looks.save(this)) this.LOOKS_SAVED = true;
    ...
    handleDamage(entity, attacker, damage, crit) {
        ...
        if (entity.effects) {
            ...
            if (G_DEBUG)
                console.info(
                    'entity.effects: ' + JSON.stringify(entity.effects)
                );

            Utils.forEach(entity.effects, function (v, k) {
                if (v === 1) effects.push(parseInt(k));
            });
        }
